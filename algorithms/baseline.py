...
class Baseline:
    def blinky_style_action(self, env, state, last_move):
        """
    ...
    def run_greedy_baseline(self, env: GhostEnv, episodes: int = 100):
        episode_rewards = []
        episode_steps = []
        ...
        for ep in range(episodes):
            ...
            last_move = None
            while not done:
                action = self.blinky_style_action(env, state, last_move)
                last_move = action
